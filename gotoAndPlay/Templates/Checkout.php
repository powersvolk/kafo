...
class Checkout extends Template {
    ...
    public static function getFormFields($fieldType = 'billing', $userType = 'user') {
        ...
        foreach ($fields as $key => $field) {
            ...
            // BF-924 make "billing_company" required
            if ($userType == 'business') {
                if ($fieldType == 'billing' && ($key == 'billing_company' || $key == 'billing_reg_code')) {
                    $fieldContext['class'] .= 'validate-required';
                    $fieldContext['input']['validate'] = 1;
                }
            }
